...
$gateVersion = (Get-Content -LiteralPath (Join-Path $repoRoot "VERSION") -Raw).Trim()
Write-Host "== HyperBoostX $gateVersion release gate =="

powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\generate_ui_action_map_v2_10.ps1 | Out-Host
if ($gateVersion -ne "2.10.0") {
...
Write-Host "Release gate complete for $gateVersion."
