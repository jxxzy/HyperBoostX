...
$corePages = @(
    ...
    @{ Key = "HardwareVendorCenter"; View = "HardwareVendorCenterView"; Generated = $true },
...
$forbiddenTemplateTokens = @(
    ...
    "Accent:",
    "Review Flow",
    "Reports, History, and Help",
    "Load Local Status",
    "Review Safe Plan",
    "Apply Approved Changes",
    "Command Rail",
    "Menu-specific evidence",
    "generated by AI",
    "AI-generated",
    "Raw JSON`n"
)
...
foreach ($page in $corePages) {
    ...
    if ($page.Generated) {
        foreach ($required in @("Operational Snapshot", "Workspace Map", "Runbook", "Evidence Actions", "Audit Console", "Advanced Details", "Safety Boundary", "Usage Notes")) {
...
foreach ($relativePath in @(
    ...
    $path = Join-Path $RepoRoot $relativePath
    if (-not (Test-Path -LiteralPath $path)) {
        $failures.Add("Missing redaction source: $relativePath")
        continue
    }
    $text = Get-Content -LiteralPath $path -Raw
    if ($text -notmatch "SensitiveTextRedactor\.Redact") {
        $failures.Add("$relativePath does not redact backend output.")
    }
    if ($text -match "Raw JSON\\n") {
        $failures.Add("$relativePath still labels backend output as raw unredacted JSON.")
    }
}
